...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/06/23 06:26:18 by psaeyang          #+#    #+#             */
/*   Updated: 2023/06/23 07:40:33 by psaeyang         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int cd(char **av, int i) {
	if (i != 2)
		return(perr("error bad aav\n"), 1);
	else if  (chdir(av[1]) == -1)
		return(perr("error cant to "), perr(av[1]), perr("\n"), 1);
	return 0;
}
...
static int exec(char **av, char **envp, int i) {
	...
	if (!pid) {
		...
		if (pip && (dup2(fds[1], 1) == -1 || close(fds[0]) == -1 || close(fds[1]) == -1))
			return(perr("error: fatal\n"), 1);
		execve(*av, av, envp);
		return(perr("error cant execute "), perr(*av), perr("\n"), 1);
	}
...
//not pass practice examshell idk where wrong