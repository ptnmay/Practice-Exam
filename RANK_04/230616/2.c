...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/06/16 03:53:52 by psaeyang          #+#    #+#             */
/*   Updated: 2023/06/16 05:13:19 by psaeyang         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int cd(char **av, int i) {
	if (i != 2)
		...
		return(perr("error: cd: can't change directory to "), perr(av[1]), perr("\n"), 1);
	 return(0);
}
...
static int exec(char **av, char **envp, int i) {
	...
	if (!pid) {
		av[i] = 0;
		if (pip && (dup2(fds[1], 1) == -1 || close(fds[0]) == -1 || close(fds[0]) == -1))
			return(perr("fatal\n"), 1);
		execve(*av, av, envp);
