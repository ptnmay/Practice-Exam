...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/06/15 23:34:35 by psaeyang          #+#    #+#             */
/*   Updated: 2023/06/16 00:02:58 by psaeyang         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int exec(char **av, char **envp, int i) {
	...
	if (!pid) {
		av[i] = 0;
		if (pip && (dup2(fds[1], 0) == -1 || close(fds[0]) == -1 || close(fds[1]) == -1))
			return(perr("error fatal\n"), 1);
		execve(*av, av, envp);
		return(perr("error: connot execute "), perr(*av), perr("\n"), 1);
	}
