...
#include <sys/select.h> // here

int max_socket; // here
int client_ids[5000]; // here
char *client_buff[5000]; // here

fd_set active, read_set, write_set; // here

char buff_read[1001]; // here
char buff_send[1001]; // here
char *msg = 0; // here
...
int main(int argc, char **argv)
{
	if (argc != 2) // here
	{
		ft_error("Wrong number of arguments"); // here
	}
	int sockfd, connfd; 
	unsigned int len; // here change len
	struct sockaddr_in servaddr, cli; 

	sockfd = socket(AF_INET, SOCK_STREAM, 0); 
	if (sockfd == -1)
		ft_error("Fatal error"); // here change error
	bzero(&servaddr, sizeof(servaddr)); 

	servaddr.sin_family = AF_INET; 
	servaddr.sin_addr.s_addr = htonl(2130706433);
	servaddr.sin_port = htons(atoi(argv[1]));  // here atoi
	...
	while(1)
	{
		...
		{
			connfd = accept(sockfd, (struct sockaddr *)&cli, &len); //keep from original main
			if (connfd < 0) //keep from original main
				ft_error("Fatal error"); //keep from original main but change error
