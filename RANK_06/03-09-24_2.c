...
void send_msg(int fd)
{
	for (int sockid = 3; sockid <= max_socket; ++sockid)
	{
		...
		{
			send_msg(sockid, buff_send, strlen(buff_send), 0);
			if (msg)
				send_msg(sockid, msg, strlen(msg), 0);
		}
	}
}
...
int main(int ac, char **av)
{
	...
	while (1)
	{
		...
		if (FD_ISSET(sockfd, &read_set))
		{
			...
			sprintf(buff_send, "servre: client %d just arrived\n", client_buff[connfd]);
			send_msg(connfd);
			continue;
		}
		for (int sockid = 3; sockid <= max_socket; ++sockid)
		{
			if (FD_ISSET(sockid, &read_set) && sockid != sockfd)
			{
				...
				{
					buff_read[read] = 0;
					client_buff[sockid] = str_join(client_buff[sockid], buff_send);
					msg = 0;
					while (extract_message(&client_buff[sockid], &msg)) //missing
					{
						sprintf(buff_send, "client %d :", client_ids[sockid]); //wrong line
						send_msg(sockid); //wrong line
						free(msg); //wrong line
						msg = 0;
					}
				}
